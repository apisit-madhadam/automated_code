...
describe("ยื่นแบบ", () => {
  ...
  it("EDN_OFFC", () => {
    ...
    const searchTexts = [
        "ข้อมูลนิติบุคคล (แบบข้อความ) (text)",
        "ข้อมูลนิติบุคคล (แบบข้อความในรูปแบบ pdf)",
        "ข้อมูลผู้ถือหุ้น (แบบข้อความ) (text)",
       "ข้อมูลผู้ถือหุ้น (แบบข้อความในรูปแบบ pdf)",
       "บัญชีรายชื่อผู้ถือหุ้น (แบบรูปภาพในรูปแบบ pdf)",
        "ข้อมูลผู้ถือหุ้น (แบบรูปภาพ) (image)",
        "ข้อมูลวัตถุประสงค์ (แบบรูปภาพในรูปแบบ pdf)"
    ]
    ...
    cy.get("#floatingTextarea").type(
      "Lorem ipsum dolor sit amet, consectetur adipiscing elit,  Gdx "
    );
    ...
    cy.get('[class="col-1 text-info"]').then(($element) => {
      if ($element.text().includes(textFromFirstTest)) {
        ...
        cy.get('[class="col-1 text-info"]')
          ...
          .click({ force: true });
        cy.get("#floatingTextarea").type("kim bass mama Gdx อนุมัติ");
        cy.get(".btn-primary").click();
        cy.get(".swal2-confirm").click();
      } else {
