...
describe("ยื่นแบบ", () => {
    ...
    it("EDN_OFFC", () => {
      ...
      cy.visit("https://edin-uat.excise.go.th/request/linkage");
      cy.get("#inlineRadio2");
      cy.wait(1000);
      ...
      const searchTexts = [
        "ข้อมูลผู้ต้องหาตามหมายจับคดีพิเศษ"
      ];
      ...
      cy.get('[class="col-1 text-info"]').then(($element) => {
        if ($element.text().includes(textFromFirstTest)) {
          ...
          cy.get('[class="col-1 text-info"]')
            ...
            .click({ force: true });
        cy.contains('ส่งกลับไปแก้ไข').click()
          cy.get("#floatingTextarea").type("ลบข้อมูล");
          cy.get('[class="btn btn-danger m-2 ng-star-inserted"]').click();
          cy.get(".swal2-confirm").click();
        } else {
          cy.log("ไม่มีใบคำขอ", textFromFirstTest);
        }
